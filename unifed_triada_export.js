...
• Art. 104.º RGIT — Fraude Fiscal Qualificada: quando a omissão excede os limiares legais.`, style: 'normal', margin: [0, 0, 0, 15] },
                ...
                // "pure-wc-finding-4" (dashboard) — alinhado com P18/P20/P21,
                // onde o valor fantasma "€458,84/mês" foi removido por falta de
                // fonte verificável e substituído por "pendente de verificação".
                // CORRIGIDO: adiciona ao PDF o mesmo enunciado do dashboard, agora
                // com "dano fiscal mensal apurado" = mediaMensalOmissao
                // (534,1475€ — valor verificado, fonte: omissaoCustos/mesesPeriodo,
                // a MESMA base que produz o checksum de 1.704.998.820,00€).
                // Nota terminológica: mediaMensalOmissao é a BASE MENSAL de
...
As plataformas procedem ao pagamento dos prestadores por transferência bancária semanal, contudo, a emissão dos documentos de reporte fiscal (extratos e faturas) ocorre em formato mensal agregado. Esta assimetria temporal constitui uma tática de ofuscação que inviabiliza a reconciliação bancária direta (cruzamento 1:1 entre extrato bancário e documento de reporte), dificultando deliberadamente auditorias financeiras e a deteção atempada das discrepâncias.`, style: 'normal', margin: [0, 0, 0, 15] },
                ...
                // ── PATCH P28 — patch_unifed_macro_v13 (1.A) ────────────────────────
                // ANTERIOR: fallback "(m.impactoSeteAnosMercado ? 50 : 50).toFixed(1)"
                // — ambos os ramos do ternário avaliam 50, divergindo do score real
                // exibido no dashboard (42/100, ASCENDENTE). O PDF omitia por completo
                // a Persistência (ATF), referida no dashboard como prova de
                // sistematicidade da conduta.
                // CORRIGIDO: usa window.NEXUS_FORECAST (definido ao abrir o modal ATF)
                // se disponível; caso contrário, fallback alinhado com o valor
                ...
                ...(function() {
                    ...
                    // ── FASE 2.1 — FIX "TENDÊNCIA: undefined" ───────────────────────────
                    // Causa raiz: quando window.NEXUS_FORECAST.valid === false (ex.: <2
                    // meses de dados), o objecto retornado por _computeForecast() não
                    // possui a propriedade 'trend', produzindo literalmente a string
                    // "undefined" no PDF. Esta correcção é apenas de apresentação:
                    // não altera o motor ATF (nexus.js) nem qualquer cálculo OLS/EMA/CV.
                    // Score "INDETERMINADO"/"INSUFFICIENT" mantém-se inalterado quando
                    // atfData.risco é 0/falsy — apenas a TENDÊNCIA passa a ter um texto
                    // explícito em vez de 'undefined'.
                    const _atfTrendFallback = isPT ? 'DADOS INSUFICIENTES (< 2 meses)' : 'INSUFFICIENT DATA (< 2 months)';
                    const _atfScoreLabel    = isPT ? 'SCORE DE PERSISTÊNCIA (SP)' : 'PERSISTENCE SCORE (SP)';
                    const _atfTrendLabel    = isPT ? 'TENDÊNCIA' : 'TREND';
                    const _atfIndeterminado = isPT ? 'INDETERMINADO' : 'INDETERMINATE';
                    const _atfMetodologia   = isPT
                        ...
                        : 'Predictive Methodology: Simple Linear Regression (OLS) combined with Exponential Moving Average (EMA). Structured omission confirmed.';
                    const atfTrendText = (atfData.trend !== undefined && atfData.trend !== null) ? atfData.trend : _atfTrendFallback;
                    const textAtf = `${_atfScoreLabel}: ${atfData.risco ? '42/100' : _atfIndeterminado}\n${_atfTrendLabel}: ${atfTrendText}\n\n${_atfMetodologia}`;
                    const _atfTitle = isPT ? 'SCORE DE PERSISTÊNCIA (ATF ENGINE)' : 'PERSISTENCE SCORE (ATF ENGINE)';
                    // ── FIM FASE 2.1 ────────────────────────────────────────────────────

                    if (atfImg) {
                        return [
                            { text: _atfTitle, style: 'h2' },
...
• Art. 30.º RGPD — Registo das Atividades de Tratamento de Dados Pessoais`, style: 'normal', margin: [0, 0, 0, 8] },
                ...
                // UNIFED_QUESTIONNAIRE.computeTopQuestions() e exibidas no Painel
                // do Advogado (motor cognitivo, score por eixo E/D/C/B/A).
                // CORRIGIDO: PDF passa a reflectir as MESMAS TOP 3 questões do
                // dashboard (m.top3Questions: axis, relevanceScore, text, norma,
                // implicacao, defesa — campos confirmados em
                ...
                ...(m.top3Questions && m.top3Questions.length > 0 ? m.top3Questions.map((q, idx) => {
                    // ── FASE 3 — i18n do conteúdo das perguntas TOP3 no PDF ─────────
                    // Usa *EN quando isPT===false e a tradução existe; caso contrário
                    // mantém o campo PT original (fallback seguro, rollout incremental).
                    const qText       = (!isPT && q.textEN)       ? q.textEN       : q.text;
                    const qNorma      = (!isPT && q.normaEN)      ? q.normaEN      : q.norma;
                    const qImplicacao = (!isPT && q.implicacaoEN) ? q.implicacaoEN : q.implicacao;
                    const qDefesa     = (!isPT && q.defesaEN)     ? q.defesaEN     : q.defesa;
                    const _axisLbl    = isPT ? 'Eixo' : 'Axis';
                    const _normaLbl   = isPT ? 'Norma Legal' : 'Legal Norm';
                    const _implLbl    = isPT ? 'Implicação' : 'Implication';
                    const _defLbl     = isPT ? 'Defesa' : 'Defense';
                    // ── FIM FASE 3 ───────────────────────────────────────────────────
                    return {
                        text: `Q${idx + 1} [${_axisLbl} ${q.axis} - Score: ${q.relevanceScore}]: ${qText}\n${_normaLbl}: ${qNorma}\n${_implLbl}: ${qImplicacao}\n${_defLbl}: ${qDefesa}\n`,
                        style: 'normal',
                        margin: [0, 0, 0, 8]
                    };
                }) : [
                    isPT ? { text: `1. [* CRÍTICA] Qual a justificação técnica para o desvio de base tributável (BTOR vs BTF) detetado na triangulação IFDE?\n2. [* CRÍTICA] Disponibilize os 'raw data' (logs de servidor) das transações anteriores ao parsing contabilístico para o período em análise.\n3. [* CRÍTICA] Forneça o 'hash chain' ou prova criptográfica que atesta a imutabilidade dos registos de faturação e logs de acesso para o período em análise.`, style: 'normal', margin: [0, 0, 0, 15] }
                          : { text: `1. [* CRITICAL] What is the technical justification for the tax base deviation (BTOR vs BTF) detected in the IFDE triangulation?\n2. [* CRITICAL] Provide the raw data (server logs) of transactions prior to accounting parsing for the period under analysis.\n3. [* CRITICAL] Provide the hash chain or cryptographic proof attesting to the immutability of the invoicing records and access logs for the period under analysis.`, style: 'normal', margin: [0, 0, 0, 15] }
                ]),
...
Fundamentação Legal: Art. 327.º CPP (Contraditório) · Art. 125.º CPP (Admissibilidade de Prova) · Art. 103.º/104.º RGIT (Fraude Fiscal/Qualificada) · Art. 36.º, n.º 11 CIVA · Decreto-Lei n.º 28/2019 (SAF-T/DAC7) · Diretiva (UE) 2021/514 (DAC7) · Termos e Condições da Plataforma · ISO/IEC 27037:2012 (prova digital)`, style: 'normal', margin: [0, 0, 0, 15] },
    ...
    async function _gerarBlobProtocoloEntrega() {
        ...
        const docDef = {
            ...
            content: [
                { text: 'PROTOCOLO DE ENTREGA FORENSE (PEF-UNIFED-01)', fontSize: 16, bold: true, alignment: 'center', margin: [0, 0, 0, 10] },
                { text: 'Âmbito: Litígio Comercial de Elevada Complexidade | Conformidade ISO/IEC 27037:2012', fontSize: 10, alignment: 'center', color: '#475569', margin: [0, 0, 0, 25] },

                { text: '1. DADOS DA EVIDÊNCIA', style: 'h2' },
                { text: `Identificador de Lote: U-${new Date().getFullYear()}-${String(new Date().getMonth()+1).padStart(2,'0')}${String(new Date().getDate()).padStart(2,'0')}-${m.session || 'SESSAO'}` },
                { text: `Data/Hora de Selagem: ${new Date().toISOString()}` },
                { text: `Master Hash SHA-256 (Manifesto): ${m.masterHash}` },
                ...
                {
                    columns: [
                        { text: '_________________________________________________\nAssinatura do Técnico Responsável Pela Análise', alignment: 'center', fontSize: 9 },
                        { text: '_________________________________________________\nAssinatura do Mandatário Judicial', alignment: 'center', fontSize: 9 }
                    ]
                }
            ],
            footer: function() {
                return { text: `Master Hash SHA-256: ${m.masterHash} (valor consistente baseado no lote de processamento).`, fontSize: 8, color: '#64748b', alignment: 'center', margin: [0, 20, 0, 0] };
            },
            styles: { h2: { fontSize: 12, bold: true, margin: [0, 10, 0, 8], color: '#1e3a8a', decoration: 'underline' } }
        };
        return await generatePDFBlob(docDef);
    }
    ...
    window._exportPacoteAdvogado = function () {
        ...
        try {
            ...
            triadaLog('info', '[FIX-TRIADA-01] A gerar 4 documentos em paralelo...');
            const [parecerBlob, custodiaBlob, peticaoBlob, protocoloBlob] = await Promise.all([
                _gerarBlobParecerAnalista(_unifiedPayload),  // passa o payload integral
                _gerarBlobAnexoCustodia(),
                _gerarPeticaoBlob(),
                _gerarBlobProtocoloEntrega()
            ]);
            ...
            if (typeof JSZip !== 'undefined') {
                ...
                // ── PATCH P31b — patch_unifed_macro_v13 (1.D) ───────────────────
                zip.file("UNIFED_PROTOCOLO_Entrega_Forense_PEF-01.pdf",  protocoloBlob);
                zip.file("UNIFED_Evidencias_Estruturado_" + sessionId + ".json", jsonBlob);
                const zipBlob = await zip.generateAsync({ type: "blob" });
