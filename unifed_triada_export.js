    ...
    function gerarConteudoAnexoCustodia(m) {
        ...
        if (m.custodyLog && m.custodyLog.length > 0) {
            ...
            m.custodyLog.slice(0, 15).forEach(log => {
                ...
                logTable.body.push([
                    ...
                    hasTimestamp ? { text: '✓ Registado', color: '#15803d' } : { text: 'SEM TIMESTAMP', color: '#b91c1c' }
                ]);
            });
        ...
        if (evidenceList.length > 0) {
            ...
            evidenceList.forEach(ev => {
                ...
                const selado = ev.sealStatusCode
                ...
                const sealStatusText = (typeof window._resolveSealStatus === 'function')
                    ? window._resolveSealStatus(ev)
                    : (ev.sealStatus || (isPT ? 'PENDENTE' : 'PENDING'));
                const sealedLabel    = isPT ? 'Selado'     : 'Sealed';
                const notSealedLabel = isPT ? 'Não Selado' : 'Not Sealed';
                evTable.body.push([
                    ev.filename || 'N/A',
                    (ev.hash || 'N/A').substring(0, 16) + '…',
                    hasTimestamp ? (ev.timestampUnix ? new Date(ev.timestampUnix * 1000).toLocaleString(lang) : (ev.timestamp || 'N/A')) : { text: isPT ? 'ÍNTEGRO (SHA-256)' : 'VERIFIED (SHA-256)', color: '#0369a1' },
                    selado ? { text: '☑ ' + sealedLabel + ' (' + (ev.sealType || 'N/D') + ')', color: '#15803d' } : { text: '☐ ' + notSealedLabel + ' — ' + sealStatusText, color: '#b91c1c' }
                ]);
            });
